...
- **修复数据完整性问题**: 对于映射表中姓名为空的记录，尝试从文件名推断姓名，避免跳过有效记录（如某学生）
...
- **修复预处理文件名匹配问题**:
  - 支持 .xlsx 扩展名文件（某学生的"早睡.xlsx"）
  - 修复全角空格文件名匹配（某学生的文件名是多个全角空格）
...
- **任务3: 支持标准 iCalendar (.ics) 日历文件导出**:
  - **批量模式**: 新增 `-ics` 参数作为开关，在正常流程后批量生成所有 ics（使用 config 中的 ics 路径）
...
5. **当功能有较大变更时，必须同步更新 README.md**（新功能、CLI参数、项目结构等）
6. **隐私策略**：所有文档（README、注释、示例代码等）中不得出现真实人名、学号等敏感信息，应使用化名（如张三、李四）或占位符
