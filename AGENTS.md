...
# 准备输入数据（两种目录结构都支持）
mkdir -p input/
# 方式一（旧版）：把收集表导出的压缩包和映射表 xlsx 直接放进 input/
# 方式二（新版）：把映射表 xlsx 放进 input/，附件解压到 input/ 下的任意子文件夹（压缩包可一并保留）
...
1. **预处理** - 扫描输入目录（压缩包或任意含课表附件的子文件夹）、按映射表重命名、映射学号；附件有更新时自动覆盖并清理旧中间结果，已失效学生的旧结果自动清理；二进制 Excel 附件跳过并列出重收名单
...
### 最新变更
- README 重构：新增「输入文件格式」说明（单个/批量散装/收集表三种方式），移除项目结构章节
...
- AI 解析支持断点重跑：已生成课程 CSV 的文件自动跳过，失败文件重跑时自动重新解析
- 适配 20260 新版输入结构：映射表 xlsx + `附件/` 文件夹（旧版「映射表 + 压缩包」仍兼容）
- 映射表不再写死列下标，改为按表头文字匹配列，兼容 5 列与 6 列布局
- 附件文件名匹配容错（全角空格 / 提交者前缀变化），压缩包内 GBK 文件名可解码
- 二进制 Excel（真 .xlsx / Excel 97-2003）附件识别并跳过，输出需重新收集名单
- 修复二维表被误判为列表格式、导致该学生被静默丢弃的问题
