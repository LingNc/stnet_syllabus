...
4. 发布版本
   ...
   当develop上累积了足够的更新，合并到main（排除plan文件夹和AGENTS.md等），打标签，完成一个版本的发布。

   **版本号规范**：`v主版本.次版本.修订版本`，例如 `v2.1.1`
   - 主版本：重大架构变更
   - 次版本：新功能添加
   - 修订版本：bug修复或小改进
...
5. 修复
   ...
   如果发布后发现有严重 bug，从 main 切出 fix/<工具名>/vx.x.x 分支进行修复，测试稳定后合并回 develop 和 main。

   ```bash
   git checkout main
   git checkout -b fix/<工具名>/vx.x.x
   # ... 修复代码 ...
   # 提交信息格式：fix: 中文描述...修复内容
   git commit -m "fix: 中文描述...修复内容"
   git checkout develop
   git merge fix/<工具名>/vx.x.x
   git checkout main
   git merge fix/<工具名>/vx.x.x
