...
- 思考模式开关（仅 OpenAI 兼容模式生效，Claude 模式暂不注入）：
  - `ai.thinking.type`: `enabled` | `disabled`（GLM-4.5+ / DeepSeek），留空则不携带该字段
  - `ai.thinking.clear_thinking`: 是否清空上一轮思考内容
  - `ai.request_body.enable_thinking`: Qwen 3.6+ 等模型的思考开关
  - 只有显式配置的字段才会写入请求体，未配置时不影响不支持该参数的模型
- AI 解析支持断点重跑：输出目录中已存在非空 `*_course.csv` 的输入文件自动跳过，失败的文件重跑时自动重新解析
...
### 增量重跑
- 重跑预处理时，附件与已有 raw 内容一致按重复跳过；内容不同视为重新提交，按修改时间取较新的一方覆盖 raw，并自动删除该学生在简化/拆分/AI 预处理/解析 CSV 各目录的旧中间文件
- 本次输入中已无对应有效附件的学生，其 raw 及下游旧中间结果自动清理，保证聚合结果与当前输入一致
